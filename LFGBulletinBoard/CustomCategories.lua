...
function Addon:GetCustomFilterKeys()
    ...
    table.sort(keys, function(a, b) 
        return savedFilters[a].sortIdx < savedFilters[b].sortIdx 
    end)
    return keys
end
...
function Addon:AddCustomFilterTags(tagListByLoc)
    isInitializedOrPanic()
    local filterKeys = self:GetCustomFilterKeys()
...
function Addon:GetAllCustomFilterLevels()
    ...
    local keys = self:GetCustomFilterKeys()
    local ranges = {}
...
local FilterSettingsPool = {
    ...
    -- Hides all option entries and spacers
    ...
    AddSpacer = function(self, relativeTo)
        local self = self ---@class FilterSettingsPool
        local marginTop = 10
        local total = #self.spacers
        local initializeSpacer = function(spacer)
            spacer:SetHeight(6)
            spacer:SetPoint("TOPLEFT", relativeTo, "BOTTOMLEFT", 0, -marginTop)
            spacer:SetPoint("RIGHT", relativeTo)
            spacer:Show()
            return spacer
        end
        for i = 1, total do
    ...
    AddCreateButton = function(self, parent)
        local self = self ---@class FilterSettingsPool
    ...
    RestorePresetsButton = function(self, parent)
        local self = self ---@class FilterSettingsPool
        ...
        addPresetsBtn:SetScript("OnClick", function()
            for key, preset in pairs(presets) do
                ...
                if saved then
                    -- Dont reset here, the preset could be hisswn because of game type like SoD, and we dont want-
...
---@param panel Frame Parent panel that the category settings will be drawn onto
function Addon:UpdateAdditionalFiltersPanel(panel)
    ---@cast Addon Addon_Options
	local userFilters = GroupBulletinBoardDB.CustomFilters
    local legend = FilterSettingsPool:AddLegend(panel)
    legend:SetPoint("RIGHT", panel:GetParent(), "RIGHT")
    legend:SetPoint("TOP", panel, "TOP")
    -- User Categories (sorted)
    local nextAnchor = panel
    FilterSettingsPool:ReleaseAll()
	for idx, key in pairs(Addon:GetCustomFilterKeys()) do
        local savedData = userFilters[key]
        local filterSettings = FilterSettingsPool:AddFilterOptions(key, panel)
        ...
        filterSettings.delete:SetScript("OnClick", function()
            StaticPopup_Show("GBB_DELETE_CATEGORY", savedData.name, nil, {
                key = savedData.key,
                panel = filterSettings:GetParent()
            })
        end)
	...
	-- New Filter 
    local createBtn = FilterSettingsPool:AddCreateButton(panel)
    if nextAnchor then
        createBtn:ClearAllPoints()
        createBtn:SetPoint("TOPLEFT", nextAnchor, "BOTTOMLEFT", 0, -25)
    end
    local addPresetsBtn = FilterSettingsPool:RestorePresetsButton(panel)
    addPresetsBtn:SetPoint("LEFT", createBtn, "RIGHT", 10, 0)
end
